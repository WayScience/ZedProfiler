...
built deterministically from the four imaging coordinates:
    ...
    patient-tumor, plate, well, field of view
...
def build_image_id(
    ...
    (patient-tumor, plate, well, field of view) so ids sort and group
    naturally by patient then plate then well then field of view.
    ...
    -------
    str
        The deterministic image id, e.g. ``"NF0014_T1_PLATE01_A1_fov1"``.

    """
    return f"{patient_tumor}_{plate}_{well}_fov{field_of_view}"
