...
_SINGLE_CHANNEL_TYPES = ("VolumeSizeShape", "Intensity", "Neighbors", "Texture", "Granularity")
...
def _parse_feature_spec(token: str) -> dict[str, object]:
    ...
    if feature_type not in ALL_FEATURE_TYPES:
        raise argparse.ArgumentTypeError(
            f"Unknown feature type {feature_type!r}; valid: {', '.join(ALL_FEATURE_TYPES)}",
        )
...
def _run_single_channel(
    ...
    if feature_type == "VolumeSizeShape":
        ...
        df = compute_neighbors(
            object_loader,
            distance_threshold=int(  # noqa: FBT003
        ...
        df = compute_texture(
            object_loader,
            distance=int(request.get("distance", 1)),  # noqa: FBT003
            grayscale=int(request.get("grayscale", 256)),  # noqa: FBT003
        )
    elif feature_type == "Granularity":
        df = compute_granularity(
            object_loader,
            radius=int(request.get("radius", 10)),  # noqa: FBT003
...
def _run_colocalization(
    ...
    df = compute_colocalization(
        two_object_loader,
        thr=int(request.get("thr", 15)),  # noqa: FBT003
...
def _resolve_requests(
    ...
    if (features_filter is None) and len(channels) >= 2:
